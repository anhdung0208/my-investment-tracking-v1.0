...
import useSWR from 'swr'; // Cài đặt bằng: npm install swr
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';
import { RefreshCw } from 'lucide-react'; // Cài đặt bằng: npm install lucide-react
import PriceCard from "../components/PriceCard";

// Giữ nguyên hàm fetcher
const fetcher = (url) => fetch(url).then((res) => res.json());

// Giữ nguyên Component CustomTooltip
...
export default function Dashboard() {
  // --- CHỈ THÊM LOGIC SWR TẠI ĐÂY ---
  const { data, error, mutate, isValidating } = useSWR('/api/market', fetcher, {
    revalidateOnFocus: false, // Không load lại khi chuyển tab
    revalidateIfStale: false,  // Ưu tiên dùng cache cũ
    dedupingInterval: 10000,  // Chống click spam trong 10s
  });

  const handleRefresh = async () => {
    await mutate(); // Ép buộc cào data mới
  };

  // Trạng thái loading ban đầu (chỉ hiện khi chưa có cache)
  if (!data && !error) {
    return (
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex justify-end">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-bold transition-all active:scale-95 text-xs uppercase tracking-wider
            ${isValidating 
